...
export default async function processEpisode(knex: Knex, episodeTmdbId: string): Promise<void> {
    let episode = await knex<EpisodeModel>("episodes").where({ tmdb_id: episodeTmdbId }).first();

    if (!episode) {
        throw new Error(`Episode with TMDB ID ${episodeTmdbId} not found`);
    }

    const show = await knex<ShowModel>("shows").where({ tmdb_id: episode.show_id }).first();

    if (!show) {
        throw new Error(`Show with TMDB ID ${episode.show_id} not found`);
    }

    if (episode.imdb_id && episode.imdb_summaries.length === 0 && episode.imdb_synopsis.length === 0) {
    ...
    if (episode.imdb_id && episode.wikipedia_url === null) {
        ...
        const wikipedia = await getWikipediaData(episode.imdb_id, show.wikidata_id, episode.season, episode.number);
    ...
    if (episode.main_plot_points.length === 0 || episode.all_plot_points.length === 0) {
        ...
        episode = await knex<EpisodeModel>("episodes").where({ tmdb_id: episodeTmdbId }).first();
    ...
    await knex<ShowModel>("shows").update({ synced_at: new Date() }).where("tmdb_id", episode.show_id);
