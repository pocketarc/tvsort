...
const extractJSONObjectResponse = (res: string): string | undefined => res.match(/\{(.|\n)*}/g)?.[0];

const extractJSONArrayResponse = (res: string): string | undefined => res.match(/\[(.|\n)*]/g)?.[0];
...
export function parseUnsafeJson(json: string): object | null {
    try {
        ...
        const extracted = (potentialArray?.length ?? 0) > (potentialObject?.length ?? 0) ? potentialArray : potentialObject;
        if (extracted) {
